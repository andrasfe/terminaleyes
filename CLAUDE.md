...
- Manual mouse actions (`/api/mouse/click_at`, `/move`, `/scroll`, `/button`) and `/api/snapshot` use a **poll-until-stable** capture loop: grab an initial frame, then poll every `TERMINALEYES_CC_POLL_INTERVAL_S` (default 1.5s) until two consecutive frames are pixel-stable (normalised MSE < `TERMINALEYES_CC_STABLE_MSE_THR`, default 0.0008) or the `TERMINALEYES_CC_MAX_WAIT_S` budget (default 15s) is exhausted. Each *changed* frame is persisted (`<label>_tN.Ns.png`) so the UI replay shows the full transition; near-duplicate frames are skipped to keep the watch dir readable. This handles both instantaneous reactions (button depress) and slow ones (page loads, app launches, modal animations) without a fixed sleep. Webcam stays open across polls; serialized with `_manual_capture_lock`; refuses while a `ControllerAgent` run holds the device.
