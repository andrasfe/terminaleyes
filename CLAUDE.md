...
- **After a host macOS upgrade, click_at lands consistently offset (e.g. 2 inches left) even though `UI Looks like` and the pointer-accel scale knob are unchanged** → host display *output* (HDMI / USB capture cable signal) was reset by the upgrade and no longer matches the capture card's native input. The card up/downscales / letterboxes, cc-frame coordinates map to a shifted region of the host's physical screen. Diagnostic: clicks at left and right both confirm with normal residuals (~0.7%) — the homer is correct relative to the streamed frame; the misalignment is upstream. Fix on host: System Settings → Displays → set the capture-device display's *Resolution* to the card's native input (typically 1920×1080 "HD" for USB capture). Not fixable from the homer side.

- **`bluetoothctl info <MAC>` shows `Connected: yes` but our HID server reports `bt_hid_connected: false`** → macOS keeps the BR/EDR control link open to paired devices but refuses to re-attach the HID L2CAP channels (PSM 17/19) after they've dropped. Pi-side `bluetoothctl connect` re-establishes BR/EDR but cannot force macOS to open HID — that decision is owned by macOS's BT HID subsystem. The Pi-side `terminaleyes-pi` reconnect watchdog (added 2026-06-01) detects this state and logs "BR/EDR up but HID not — macOS won't auto-open HID L2CAP from Pi side" instead of pointlessly retrying. Recovery requires Mac-side action: toggle Bluetooth off/on, click Connect in System Settings, OR sleep/wake the Mac. The auto-trust handler in `scripts/bt-agent.py` (also added 2026-06-01) prevents the original-pair `Trusted: no` variant, but cannot fix this BR/EDR-up-HID-down state.

- **After re-pair the Mac's bond record shows `Trusted: no` and HID never auto-attaches on next session** → bt-agent.py's auto-trust hook (added 2026-06-01) listens for `Device1.PropertiesChanged` with `Paired=True` and sets `Trusted=True` via D-Bus. Without it, every pair leaves Trusted off, and some macOS versions refuse to auto-open HID L2CAP for non-trusted devices. If the agent isn't running (check `systemctl status bt-agent.service`) the trust has to be set manually with `sudo bluetoothctl trust <MAC>`.
