...
- `POST /api/snapshot?dedup=1` (used by the UI's **Active Refresh** checkbox) skips the poll loop entirely: it grabs a single frame and persists it only when the same change-fraction metric says it differs from the most recent stored frame. The UI poller fires this every 60s while the tab is visible and the checkbox is armed, so an idle screen produces zero writes.
